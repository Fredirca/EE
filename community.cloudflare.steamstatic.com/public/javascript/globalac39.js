	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			else
			{
				...
				strURL += '?content_only=1&omit_owner=1&l=latam';
			...
			if ( json.success == 1 )
			{
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Actualizar descriptores de contenido', 'Ocurrió un problema al actualizar los descriptores de contenido para este artículo: ' + json.success );
						}
					} );
				} );
			}
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marcar como sospechoso', 'Esto aplicará un bloqueo de la comunidad al autor, agregará este comentario como datos de entrenamiento para que podamos marcar este tipo de comentarios automáticamente y borrará todos los comentarios del autor. ¿Seguro que es lo que quieres hacer?', 'Marcar como sospechoso' );
		dialog.done( function() {
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		{
			var strModalBody = 'Estás a punto de marcar esta publicación como la respuesta a este hilo. Esto indicará que la cuestión original ha sido respondida y enlazará a esta publicación específica. Puedes eliminar esta marca o indicar una publicación diferente como respuesta en cualquier momento si cambias de opinión.';
			if ( bExisting )
				strModalBody = 'Ya se ha seleccionado una publicación diferente como respuesta a este hilo. ¿Quieres elegir esta publicación como la nueva respuesta?';
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'Se produjo un error de comunicación con la red. Inténtalo de nuevo más tarde.';
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Compartir', 'La actualización de estado ha sido publicada en Actividad de amigos.' );
		},
		onFailure: function(transport) {
