...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Спасибо!', 'Спасибо, что отправили жалобу. Благодаря вам сообщество Steam остаётся приятным и дружелюбным.' );
	}).fail( function() {
		ShowAlertDialog( 'Пожаловаться', 'При попытке пожаловаться произошла ошибка. Пожалуйста, попробуйте позже.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Внимание: ссылка, по которой вы переходите, не ведет на официальный сайт Steam.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Если на этом сайте вас попросят ввести свой логин или пароль, ни при каких условиях не делайте этого. Вы можете лишиться своего аккаунта в Steam и всех купленных игр!\n'
						+ 'Вы уверены, что хотите перейти на эту страницу? Нажмите ОК, чтобы продолжить на свой страх и риск.\n' );
	}

	ShowAlertDialog( '', 'Ошибка в адресе ссылки.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Пожаловаться', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=russian';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=russian';
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Изменить язык', 'При подключении к серверам Steam возникла проблема. Повторите попытку позже.' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Обновить дескрипторы контента', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Обновить дескрипторы контента', 'При обновлении дескрипторов этого контента произошла ошибка: ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Изменить настройки' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Судя по вашим настройкам, контент может вам не подойти '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Подписаться', 'При обновлении подписки произошла ошибка. Пожалуйста, повторите попытку позже.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Подписаться', 'При обновлении подписки произошла ошибка. Пожалуйста, повторите попытку позже.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Введите здесь свой комментарий.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Пометить как подозрительное', 'Автор будет заблокирован в сообществе, и все его комментарии будут удалены. Наша система проанализирует этот комментарий, чтобы скрывать подобный контент автоматически. Вы уверены, что хотите сделать это?', 'Пометить как подозрительное' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Снять пометку о блокируемом контенте', 'Вы уверены, что хотите снять пометку о блокируемом контенте с этого комментария? Это действие нельзя отменить.', 'Снять пометку о блокируемом контенте' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Удалить ответ', 'Вы уверены, что это сообщение не является ответом на обсуждение? Решение можно будет изменить.', 'Удалить ответ' );
		}
		else
		{
			var strModalBody = 'Вы хотите сделать это сообщение ответом на тему? В ней появится пометка об ответе и отдельная ссылка на него. Пометку можно будет удалить или переместить на другое сообщение.';
			if ( bExisting )
				strModalBody = 'В этом обсуждении уже выбран ответ. Вы уверены, что хотите сделать это сообщение новым ответом?';

			$modal = ShowConfirmDialog( 'Отметить как ответ', strModalBody, 'Выбрать ответ' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Не удалось изменить ответ. Ошибка: ' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'У вас нет разрешения обновить ответ этой темы.'
						}

						ShowAlertDialog( 'Ошибка', strError );
					}
				},
	...
	{
		var strMessage = 'Простите, что-то пошло не так: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Ошибка соединения с сетью. Пожалуйста, повторите попытку позже.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Подписаться', 'Теперь вы будете получать уведомления о новых комментариях.');
			}, function() {
				ShowAlertDialog('Подписаться', 'При обновлении подписки произошла ошибка. Пожалуйста, повторите попытку позже.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Отписаться', 'Вы больше не будете получать уведомления о новых комментариях.');
			}, function() {
				ShowAlertDialog('Отписаться', 'При обновлении подписки произошла ошибка. Пожалуйста, повторите попытку позже.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Форматирование текста', data );
	});
};
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Поделиться', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Поделиться', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Поделиться', 'Статус размещён в ленте активности.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Поделиться', 'Не удалось разместить статус. Пожалуйста, повторите попытку позже.' );
		}
	});
}
