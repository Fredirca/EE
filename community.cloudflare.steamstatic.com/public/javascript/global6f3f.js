...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( '위반 신고', '귀하의 보고를 저장하는 데 문제가 발생하였습니다. 나중에 다시 시도해 주세요.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( '경고: 누르신 URL은 공식 Steam 웹사이트가 아닙니다.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ '웹사이트에서 사용자 이름이나 암호를 요구한다면 해당 정보를 입력하지 마세요. Steam 계정과 등록된 게임들을 잃어버리게 될 수도 있습니다!\n'
						+ '정말 이 페이지를 방문할까요? 위험함을 인지하였고 계속 진행하려면 확인을 클릭하세요.\n' );
	}

	ShowAlertDialog( '', 'URL 형식이 잘못되었습니다.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( '위반 신고', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=koreana';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=koreana';
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( '언어 변경', 'Steam 서버와 연결하는 데 문제가 있습니다. 나중에 다시 시도해 주세요.' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( '콘텐츠 설명 업데이트', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( '콘텐츠 설명 업데이트', '이 아이템에 대한 콘텐츠 설명을 업데이트하는 도중 문제가 발생했습니다. ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( '환경 설정 수정' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': '환경 설정에서 표시하지 않기로 설정한 콘텐츠가 포함되어 있습니다. '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('글타래 구독', '구독 내용을 갱신하는 중 오류가 발생했습니다. 나중에 다시 시도해주세요.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('글타래 구독', '구독 내용을 갱신하는 중 오류가 발생했습니다. 나중에 다시 시도해주세요.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', '게시할 댓글을 입력해주세요.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( '의심스러운 콘텐츠로 표시', '작성자에게 커뮤니티 차단을 적용하고, 이 댓글을 트레이닝 데이터로 추가하여 이러한 유형의 댓글에 자동으로 깃발 표시를 적용하며 작성자의 모든 댓글을 삭제하도록 합니다. 계속 진행할까요?', '의심스러운 콘텐츠로 표시' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( '차단 가능 콘텐츠의 깃발 표시 제거', '댓글에서 차단 가능 콘텐츠의 깃발 표시를 제거합니다. 이 작업은 취소할 수 없습니다. 깃발 표시를 제거할까요?', '차단 가능 콘텐츠의 깃발 표시 제거' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( '답변으로 표시 제거', '게시물이 글타래의 답변으로 표시되는 것을 해제할까요? 이 설정은 언제든지 업데이트할 수 있습니다.', '답변으로 표시 제거' );
		}
		else
		{
			var strModalBody = '이 게시물을 이 글타래의 답변으로 표시하려 합니다. 그 경우 원본 게시물 및 이 게시물로의 링크는 답변이 된 것으로 표시됩니다. 마음이 바뀌실 경우 언제든지 이를 제거하거나 다른 게시물을 답변으로 표시할 수 있습니다.';
			if ( bExisting )
				strModalBody = '이미 답변으로 채택된 다른 게시물이 있습니다. 이 게시물을 새 답변으로 채택할까요?';

			$modal = ShowConfirmDialog( '답변으로 표시', strModalBody, '답변 선택' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = '이 주제의 답변을 업데이트하는 데 오류가 발생했습니다. 오류:' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = '이 주제의 답변을 변경할 권한이 없습니다.'
						}

						ShowAlertDialog( '오류', strError );
					}
				},
	...
	{
		var strMessage = '죄송합니다. 오류가 발생했습니다. ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += '네트워크와 통신하는 동안 문제가 발생했습니다. 나중에 다시 시도해 주세요.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('글타래 구독', '누군가 이 글타래에 답변을 달면 댓글 알림을 받게 됩니다.');
			}, function() {
				ShowAlertDialog('글타래 구독', '구독 내용을 갱신하는 중 오류가 발생했습니다. 나중에 다시 시도해주세요.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('글타래 구독 취소', '이 글타래에서 더 이상 댓글 알림을 받지 않습니다.');
			}, function() {
				ShowAlertDialog('글타래 구독 취소', '구독 내용을 갱신하는 중 오류가 발생했습니다. 나중에 다시 시도해주세요.');
			});
		});
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( '공유하기', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  '공유하기', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( '공유하기', '친구 활동에 상태 업데이트가 게시되었습니다.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( '공유하기', '상태 업데이트 공유에 문제가 발생했습니다. 잠시 후 다시 시도하세요.' );
		}
	});
}
