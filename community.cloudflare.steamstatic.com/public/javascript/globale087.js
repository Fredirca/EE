...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Vælg en grund til, at der anmeldes en overtrædelse' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Anmeld overtrædelse', 'Der opstod et problem i forsøget på at gemme din rapportering. Prøv venligst igen senere.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Bemærk: URL\'en, du har klikket på, er ikke en officiel Steam-webside.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Hvis websiden beder om dit brugernavn eller adgangskode, skal du ikke indtaste den information. Du kunne miste din Steam-konto og alle dine spil!\n'
						+ 'Er du sikker på, at du vil besøge denne side? Klik OK for at fortsætte på egen risiko.\n' );
	}

	ShowAlertDialog( '', 'URL\'en er skrevet forkert.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Anmeld overtrædelse', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=danish';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=danish';
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Opdater indholdsbeskrivelser', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Opdater indholdsbeskrivelser', 'Der opstod et problem med at opdatere indholdsbeskrivelser for dette emne: ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Rediger præferencer' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Indholdet er muligvis ikke passende baseret på dine præferencer '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Abonner på tråd', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Abonner på tråd', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Skriv venligst en kommentar.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marker som mistænkeligt', 'Dette giver en fællesskabsudelukkelse til forfatteren. Tilføj denne kommentar som "læringsdata", så vi kan markere denne slags kommentarer automatisk og rydde alle forfatterens kommentarer. Er du sikker på, at du ønsker at gøre dette?', 'Marker som mistænkeligt' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Fjern markeringen "Forbudt indhold"', 'Dette fjerner markeringen om forbudt indhold i denne kommentar og kan ikke fortrydes. Er du sikker på, at du vil fortsætte?', 'Fjern markeringen "Forbudt indhold"' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Fjern som svar', 'Er du sikker på, at du vil markere dette indlæg som svaret til denne tråd? Du kan opdatere dette på et hvert tidspunkt.', 'Fjern som svar' );
		}
		else
		{
			var strModalBody = 'Du er ved at markere dette indlæg som svaret til denne tråd. Dette vil angive, at det oprindelige indlæg er blevet besvaret og linke til dette specifikke indlæg. Du kan til enhver tid fjerne dette eller markere et andet indlæg som svaret, hvis du skifter mening.';
			if ( bExisting )
				strModalBody = 'Et svar er allerede blevet valgt som svaret til denne tråd. Vil du gerne vælge dette indlæg som det nye svar til denne tråd?';

			$modal = ShowConfirmDialog( 'Marker som svar', strModalBody, 'Vælg svar' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Der opstod et problem med at opdatere dette emnesvar. Fejl:' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'Du har ikke tilladelser til at opdatere dette emnes svar.'
						}

						ShowAlertDialog( 'Fejl', strError );
					}
				},
	...
	{
		var strMessage = 'Beklager, der er sket en form for fejl: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Der opstod en fejl under kommunikationen med netværket. Prøv igen senere.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Abonner på tråd', 'Du modtager en notifikation, når nogen skriver i disse kommentarer.');
			}, function() {
				ShowAlertDialog('Abonner på tråd', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Opsig abonnement på kommentarer', 'Du modtager ikke længere notifikationer fra disse kommentarer.');
			}, function() {
				ShowAlertDialog('Opsig abonnement på kommentarer', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Del', 'Denne statusopdatering er blevet slået op på din venneaktivitet.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Del', 'Der var et problem under delingen af statusopdateringen. Prøv venligst igen senere.' );
		}
	});
}
