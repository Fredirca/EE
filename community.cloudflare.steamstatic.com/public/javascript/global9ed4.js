...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Veuillez spécifier la nature de l\'abus à signaler' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Merci !', 'Nous vous remercions d\'avoir signalé du contenu inapproprié. Grâce à vous, nous allons pouvoir préserver le caractère convivial et civilisé de la communauté Steam.' );
	}).fail( function() {
		ShowAlertDialog( 'Signaler une violation', 'Un problème est survenu lors de l\'enregistrement de votre signalement. Veuillez réessayer plus tard.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Note : le lien sur lequel vous venez de cliquer ne correspond pas à un site internet officiel Steam.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Si ce site internet vous demande votre nom de compte ou votre mot de passe, ne fournissez surtout pas ces informations : vous pourriez perdre votre compte Steam et vos jeux !\n'
						+ 'Voulez-vous vraiment visiter cette page ? Cliquez sur OK pour continuer à vos risques et périls.\n' );
	}

	ShowAlertDialog( '', 'La syntaxe de cette URL est incorrecte.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Signaler une violation', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=french';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=french';
...
{
	var Modal = ShowBlockingWaitDialog( 'Changer la langue', '' );
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Changer la langue', 'Un problème de communication avec les serveurs Steam s\'est produit. Veuillez réessayer plus tard.' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Mettre à jour les descripteurs de contenu', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Mettre à jour les descripteurs de contenu', 'Un problème est survenu pendant la mise à jour de ces descripteurs de contenu : ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Préférences' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Ce contenu peut ne pas être approprié d\'après vos préférences '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('S\'abonner au fil de discussion', 'Une erreur est survenue lors de la mise à jour de votre abonnement. Veuillez réessayer ultérieurement.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('S\'abonner au fil de discussion', 'Une erreur est survenue lors de la mise à jour de votre abonnement. Veuillez réessayer ultérieurement.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Veuillez saisir un commentaire pour poster.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Définir comme suspect', 'Cette action entrainera le bannissement de l\'auteur(e) de la communauté, ajoutera ce commentaire aux données d\'apprentissage pour que nous puissions appliquer automatiquement un signalement à ce type de commentaires et éliminera tous les commentaires de l\'auteur(e). Voulez-vous vraiment continuer ?', 'Définir comme suspect' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Supprimer le signalement pour contenu à bannir', 'Cette action supprimera le signalement pour contenu à bannir sur ce commentaire, et elle est irréversible. Voulez-vous vraiment continuer ?', 'Supprimer le signalement pour contenu à bannir' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Supprimer cette réponse', 'Voulez-vous vraiment supprimer ce message comme réponse à ce sujet ? Vous pourrez le mettre à jour à tout moment.', 'Supprimer cette réponse' );
		}
		else
		{
			var strModalBody = 'Vous êtes sur le point de sélectionner ce message en tant que réponse à ce sujet. Cela indiquera que ce message répond à votre question. Vous pouvez annuler cette action ou sélectionner un autre message à tout moment si vous changez d\'avis.';
			if ( bExisting )
				strModalBody = 'Une réponse a déjà été sélectionnée. Voulez-vous choisir ce message comme réponse à ce sujet ?';

			$modal = ShowConfirmDialog( 'Choisir comme réponse', strModalBody, 'Choisir la réponse' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Une erreur est survenue lors de la mise à jour de la réponse à ce sujet. Erreur :' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'Vous n\'avez pas la permission de mettre à jour la réponse à ce sujet.'
						}

						ShowAlertDialog( 'Erreur', strError );
					}
				},
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'Une erreur de communication avec le réseau s\'est produite. Veuillez réessayer ultérieurement.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('S\'abonner au fil de discussion', 'Vous recevrez une notification à chaque fois qu\'une personne répondra à ce fil de discussion.');
			}, function() {
				ShowAlertDialog('S\'abonner au fil de discussion', 'Une erreur est survenue lors de la mise à jour de votre abonnement. Veuillez réessayer ultérieurement.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Supprimer l\'abonnement', 'Vous ne recevrez plus de notifications liées à ce fil de discussion.');
			}, function() {
				ShowAlertDialog('Supprimer l\'abonnement', 'Une erreur est survenue lors de la mise à jour de votre abonnement. Veuillez réessayer ultérieurement.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Formatage du texte', data );
	});
};
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Partager', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Partager', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Partager', 'Le message de statut a été posté dans votre flux d\'activité.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Partager', 'Une erreur est survenue lors du partage du message de statut. Veuillez réessayer plus tard.' );
		}
	});
}
