...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'ありがとうございます！', '違反行為をご報告いただき、ありがとうございます。Steam コミュニティでは公正でフレンドリーな環境を提供できるよう努めています。' );
	}).fail( function() {
		ShowAlertDialog( '違反行為の報告', '報告の保存中にエラーが発生しました。後でもう一度お試しください。' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( '注意: クリックした URL は公式 Steam Web サイトではありません。\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'この Web サイトでユーザー名やパスワードを尋ねられたとしても、絶対に入力しないでください。  Steam アカウントや所有しているゲームが失われる可能性があります！\n'
						+ '本当にこのページを開いてもよろしいですか？OKをクリックすると自己責任でページを開きます。\n' );
	}

	ShowAlertDialog( '', 'URLの形式が間違っています。');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( '違反行為の報告', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=japanese';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=japanese';
...
{
	var Modal = ShowBlockingWaitDialog( '言語を変更', '' );
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( '言語を変更', 'Steam サーバーとの通信中に問題が発生しました。 後でもう一度お試しください。' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'コンテンツ記述子を更新', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'コンテンツ記述子を更新', 'コンテンツ記述子を更新する際に問題が発生しました：' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( '個人設定を編集' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': '個人設定で指定した不適切なコンテンツの可能性があります '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('スレッドを購読する', 'サブスクリプションの更新中にエラーが発生しました。後でもう一度お試しください。');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('スレッドを購読する', 'サブスクリプションの更新中にエラーが発生しました。後でもう一度お試しください。');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'コメントを入力して投稿してください。' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( '「疑わしい」としてマークを付ける', 'これにより、投稿者にはコミュニティ禁止が課され、投稿者のコメント全てが消去されます。また、この種のコメントに自動的にフラグを付けるためのトレーニングデータとしても使用されます。実行してよろしいですか？', '「疑わしい」としてマークを付ける' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( '禁止コンテンツフラグをクリア', 'これにより、禁止コンテンツフラグはクリアされます。この処理を取り消すことはできません。実行してよろしいですか？', '禁止コンテンツフラグをクリア' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( '回答マークを外す', 'この投稿からこのスレッドに対する回答マークを外しますか? これはいつでも変更可能です。', '回答マークを外す' );
		}
		else
		{
			var strModalBody = 'スレッドに対する回答マークをこの投稿に付けようとしています。マークを付けると、オリジナルの投稿は回答済みとされ、この投稿へ直接リンクされます。後から回答マークを外したり、別の投稿に付け直したりすることも可能です。';
			if ( bExisting )
				strModalBody = 'このスレッドへの回答として別の投稿が既に選択されています。この投稿を新たな回答として選択しますか?';

			$modal = ShowConfirmDialog( '回答マークを付ける', strModalBody, '回答を選択する' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'このトピックの回答を更新する際に問題が発生しました。エラー: ' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'このトピックの回答を更新する権限がありません。'
						}

						ShowAlertDialog( 'エラー', strError );
					}
				},
	...
	{
		var strMessage = '申し訳ありません、エラーが発生しました: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'ネットワークとの通信中に問題が発生しました。後でもう一度お試しください。';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('スレッドを購読する', 'このスレッドに誰かが返信する度、コメントのお知らせを受け取ります。');
			}, function() {
				ShowAlertDialog('スレッドを購読する', 'サブスクリプションの更新中にエラーが発生しました。後でもう一度お試しください。');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('スレッドの購読を解除する', '以後このスレッドでコメントのお知らせは受け取りません。');
			}, function() {
				ShowAlertDialog('スレッドの購読を解除する', 'サブスクリプションの更新中にエラーが発生しました。後でもう一度お試しください。');
			});
		});
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( '共有', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  '共有', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( '共有', 'フレンドアクティビティにコメントが投稿されました。' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( '共有', 'コメントの共有時に問題が発生しました。 後でもう一度お試しください。' );
		}
	});
}
