...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Let op: de link waar je op hebt geklikt leidt niet naar een officiële Steam-website.\n\n'
						...
						+ 'Geef nooit je gebruikersnaam of wachtwoord op als deze website daarnaar vraagt. Je zou je Steam-account ermee kunnen verliezen - en daarmee ook al je spellen!\n'
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Taal wijzigen', 'Er is een probleem opgetreden bij het communiceren met de Steam-servers. Probeer het later opnieuw.' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Beschrijving inhoud bijwerken', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Beschrijving inhoud bijwerken', 'Er is een probleem opgetreden bij het bijwerken van de beschrijving van de inhoud van dit item: ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Voorkeuren bewerken' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Inhoud is mogelijk niet geschikt op basis van je voorkeuren '} ) );

			var $elOptions = $J( '<div></div>' );
			var $elViewOption = $J( '<div></div>', {
				'class': 'ugc_inline_option',
				'text': 'Inhoud weergeven'
			} );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markeren als verdacht', 'Hierdoor wordt de auteur ervan verbannen uit de Community, wordt deze reactie toegevoegd aan de trainingsgegevens voor ons algoritme zodat we dit type reacties in de toekomst automatisch kunnen markeren, en worden alle reacties van deze auteur gewist. Weet je zeker dat je dit wilt doen?', 'Markeren als verdacht' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markering als verboden inhoud opheffen', 'Weet je zeker dat je de markering \'verboden inhoud\' voor deze reactie wilt wissen? Dit kan niet ongedaan worden gemaakt.', 'Markering als verboden inhoud opheffen' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Verwijderen als antwoord', 'Weet je zeker dat je dit bericht niet meer wilt markeren als het antwoord op dit onderwerp? Je kunt dit op elk gewenst moment bijwerken.', 'Verwijderen als antwoord' );
		}
		else
		{
			var strModalBody = 'Je staat op het punt dit bericht te markeren als het antwoord op dit onderwerp. Dit zal aangeven dat het oorspronkelijke bericht beantwoord is en naar dit specifieke bericht verwijzen. Je kunt dit verwijderen of op elk gewenst moment een ander bericht als het antwoord aangeven, mocht je van gedachten veranderen.';
			if ( bExisting )
				strModalBody = 'Een ander bericht is al als antwoord op dit onderwerp geselecteerd. Wil je dit bericht kiezen als het nieuwe antwoord?';

			$modal = ShowConfirmDialog( 'Markeren als antwoord', strModalBody, 'Antwoord kiezen' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Er is een probleem opgetreden bij het bijwerken van dit onderwerpantwoord. Fout: ' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'Je hebt niet voldoende rechten om het antwoord van dit onderwerp bij te werken.'
						}

						ShowAlertDialog( 'Fout', strError );
					}
				},
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Uitschrijven van reacties', 'Je zult niet langer meldingen van reacties hierop ontvangen.');
			}, function() {
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Delen', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
