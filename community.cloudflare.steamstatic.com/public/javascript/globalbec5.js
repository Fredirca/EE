...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Välj en anledning till rapporteringen av missbruk' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Anmäl kränkning', 'Det uppstod ett problem när din anmälan skulle sparas. Var god försök igen senare.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Obs: URLen du har klickat på är inte en officiell Steam-webbplats.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Om denna webbplats frågar efter ditt användarnamn eller lösenord, ange inte den informationen. Du kan förlora ditt Steam-konto och alla dina spel!\n'
						+ 'Är du säker på att du vill besöka den här sidan? Klicka OK för att fortsätta på egen risk.\n' );
	}

	ShowAlertDialog( '', 'URLen är felaktigt utformad.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Anmäl kränkning', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=swedish';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=swedish';
...
{
	var Modal = ShowBlockingWaitDialog( 'Byt språk', '' );
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Byt språk', 'Det uppstod ett fel vid kommunikationen med Steam-servrarna. Vänligen försök igen senare.' );
		});
}
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Uppdatera innehållsbeskrivningar', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Uppdatera innehållsbeskrivningar', 'Det uppstod ett problem vid uppdateringen av innehållsbeskrivningarna för detta innehåll: ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Redigera preferenser' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Innehållet kan vara olämpligt för dig baserat på dina preferenser '} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Prenumerera på tråd', 'Det uppstod ett problem vid uppdatering av abonnemang. Försök igen senare.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Prenumerera på tråd', 'Det uppstod ett problem vid uppdatering av abonnemang. Försök igen senare.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Skriv en kommentar att lägga upp.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markera som misstänkt', 'Detta utdelar en gemenskapsavstängning till skaparen. Vi lägger även till den här kommentaren i vår inlärningsdata så att vi automatiskt kan flagga på den här typen av kommentarer och radera alla kommentarer från skaparen i framtiden. Är du säker på att du vill fortsätta?', 'Markera som misstänkt' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Ta bort markering för förbjudet innehåll', 'Denna åtgärd tar bort flaggan för förbjudet innehåll för denna kommentar och kan inte ångras. Är du säker på att du vill fortsätta?', 'Ta bort markering för förbjudet innehåll' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Ta bort som svar', 'Är du säker på att du vill avmarkera detta inlägg som svar på frågan i denna tråd? Detta kan ändras när som helst.', 'Ta bort som svar' );
		}
		else
		{
			var strModalBody = 'Du är på väg att markera det här inlägget som svar på denna tråd. Detta kommer indikera att det ursprungliga inlägget har besvarats och länka till det här inlägget. Du kan ta bort det, eller sätta ett annat inlägg som svar när som helst om du ändrar dig.';
			if ( bExisting )
				strModalBody = 'Ett annat inlägg har redan markerats som svar på denna tråd. Vill du välja detta inlägg som det nya svaret?';

			$modal = ShowConfirmDialog( 'Markera som svar', strModalBody, 'Välj svar' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Ett fel uppstod när detta ämnessvar skulle uppdateras. Fel: ' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'Du har inte behörighet att uppdatera detta ämnes svar.'
						}

						ShowAlertDialog( 'Fel', strError );
					}
				},
	...
	{
		var strMessage = 'Tyvärr har något sorts fel inträffat: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Ett fel uppstod i kommunikationen med nätverket. Försök igen senare.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Prenumerera på tråd', 'Du kommer att få ett kommentarsmeddelande när någon svarar på denna tråd.');
			}, function() {
				ShowAlertDialog('Prenumerera på tråd', 'Det uppstod ett problem vid uppdatering av abonnemang. Försök igen senare.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Sluta prenumerera på tråd', 'Du kommer inte längre få kommentarsmeddelanden från denna tråd.');
			}, function() {
				ShowAlertDialog('Sluta prenumerera på tråd', 'Det uppstod ett problem vid uppdatering av abonnemang. Försök igen senare.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Textformatering', data );
	});
};
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Dela', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Dela', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Dela', 'Statusuppdateringen har lagts upp på din Vänaktivitet.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Dela', 'Det uppstod ett problem vid försök att dela statusuppdateringen. Var god försök igen senare.' );
		}
	});
}
