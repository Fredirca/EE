...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Scegli un motivo per segnalare un abuso' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Grazie!', 'Grazie per aver segnalato un contenuto offensivo, aiutando così a mantenere la Comunità di Steam un luogo sereno e amichevole.' );
	}).fail( function() {
		ShowAlertDialog( 'Segnala violazione', 'Si è verificato un problema durante il salvataggio della segnalazione. Riprova più tardi.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Nota: l\'URL che hai cliccato non è un sito web ufficiale di Steam.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Se il sito in questione dovesse chiederti nome utente o password, non fornire tali informazioni. Potresti perdere l\'accesso al tuo account di Steam e a tutti i tuoi giochi!\n'
						+ 'Sei sicuro di voler visitare questa pagina? Clicca OK per continuare a tuo rischio e pericolo.\n' );
	}

	ShowAlertDialog( '', 'L\'URL è formattato incorrettamente.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Segnala violazione', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			if ( appid == 'classinfo' )
			{
				...
				strURL += '?content_only=1&l=italian';
			}
			else
			{
				// real asset
				...
				strURL += '?content_only=1&omit_owner=1&l=italian';
	...
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxeditcontentdescriptors/', { publishedfileid: publishedfileid } )
		.done( function( json ) {
			if ( json.success == 1 )
			{
				...
				var dialog = ShowConfirmDialog( 'Aggiorna descrittori contenuto', content );
				...
				dialog.done( function() {
					...
					).done( function( json )
					{
						...
						{
							ShowAlertDialog( 'Aggiorna descrittori contenuto', 'Si è verificato un problema durante l\'aggiornamento dei descrittori contenuto per questo elemento: ' + json.success );
						}
					} );
				} );
			}
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// preferences
	{
		...
		var $elViewPreferences = $J( '<div/>' ).click( fnViewPreferences ).text( 'Modifica preferenze' ).addClass( 'option' );
...
function ApplyAdultContentPreferencesHelper( e, rgContentDescriptorsToExclude, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'I contenuti potrebbero non essere appropriati sulla base delle tue preferenze '} ) );

			var $elOptions = $J( '<div></div>' );
			var $elViewOption = $J( '<div></div>', {
				'class': 'ugc_inline_option',
				'text': 'Visualizza contenuto'
			} );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Segui discussione', 'Non siamo riusciti ad aggiornare la tua sottoscrizione. Riprova più tardi.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Segui discussione', 'Non siamo riusciti ad aggiornare la tua sottoscrizione. Riprova più tardi.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Scrivi un commento.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Contrassegna come sospetto', 'L\'autore sarà bandito dalla Comunità, il commento sarà aggiunto ai dati che usiamo per il miglioramento della funzione per contrassegnare automaticamente questo tipo di commenti e tutti i commenti dell\'autore saranno rimossi. Confermi?', 'Contrassegna come sospetto' );
		dialog.done( function() {
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Rimuovi il contrassegno di contenuto passibile di ban', 'Il contrassegno di contenuto passibile di ban sarà rimosso da questo commento. Questa azione è definitiva. Confermi?', 'Rimuovi il contrassegno di contenuto passibile di ban' );
		dialog.done( function() {
			thread.m_bLoading = true;
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( 'Rimuovi come risposta', 'Sei sicuro di non voler più indicare il messaggio come risposta a questa discussione? Potrai cambiare la tua scelta in qualsiasi momento.', 'Rimuovi come risposta' );
		}
		else
		{
			var strModalBody = 'Stai per scegliere questo messaggio come risposta alla discussione. Se decidi di procedere, questo messaggio specifico verrà indicato come risposta nel messaggio originale della discussione. La risposta può essere rimossa o cambiata in qualsiasi momento.';
			if ( bExisting )
				strModalBody = 'È già stato selezionato un messaggio diverso come risposta a questa discussione. Vuoi comunque scegliere questo messaggio come nuova risposta a questa discussione?';

			$modal = ShowConfirmDialog( 'Scegli come risposta', strModalBody, 'Scegli risposta' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Si è verificato un errore durante l\'aggiornamento della risposta per questa discussione. Errore:' + transport.responseJSON.success;
						if ( transport.responseJSON.success == 15 )
						{
							strError = 'Non hai i permessi necessari per aggiornare la risposta a questa discussione.'
						}

						ShowAlertDialog( 'Errore', strError );
					}
				},
	...
	{
		var strMessage = 'Spiacente, qualcosa è andato storto: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Si è verificato un errore durante la comunicazione con la rete. Riprova più tardi.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Segui discussione', 'Riceverai una notifica quando qualcuno fa un commento in questa discussione.');
			}, function() {
				ShowAlertDialog('Segui discussione', 'Non siamo riusciti ad aggiornare la tua sottoscrizione. Riprova più tardi.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Smetti di seguire discussione', 'Non riceverai più notifiche quando qualcuno fa commenti in questa discussione.');
			}, function() {
				ShowAlertDialog('Smetti di seguire discussione', 'Non siamo riusciti ad aggiornare la tua sottoscrizione. Riprova più tardi.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Formattazione del testo', data );
	});
};
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Condividi', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Condividi', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Condividi', 'L\'aggiornamento di stato è stato pubblicato sulla tua Attività degli amici.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Condividi', 'Si è verificato un problema durante la condivisione dell\'aggiornamento di stato. Si prega di riprovare più tardi.' );
		}
	});
}
