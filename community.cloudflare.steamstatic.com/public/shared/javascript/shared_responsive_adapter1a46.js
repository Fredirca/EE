...
jQuery( function($) {
	...
	if ( $LocalMenuContent.length )
	{
		...
		GPNavUpdateActionDescriptions( $('#responsive_page_template_content'), { onOptionsActionDescription:'เมนู' } );
	...
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "thai"} ).text( 'ไทย (Thai)' )
					...
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( '简体中文 (จีนตัวย่อ)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( '繁體中文 (จีนตัวเต็ม)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "japanese"} ).text( '日本語 (ญี่ปุ่น)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "koreana"} ).text( '한국어 (เกาหลี)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "bulgarian"} ).text( 'български (บัลแกเรีย)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Čeština (เช็ก)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Dansk (เดนมาร์ก)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Deutsch (เยอรมัน)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'English (อังกฤษ)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español - España (สเปน)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Español - Latinoamérica (สเปน - ลาตินอเมริกา)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "greek"} ).text( 'Ελληνικά (กรีก)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Français (ฝรั่งเศส)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Italiano (อิตาลี)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Magyar (ฮังการี)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Nederlands (ดัตช์)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Norsk (นอร์เวย์)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Polski (โปแลนด์)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Português (โปรตุเกส - โปรตุเกส)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Português - Brasil (โปรตุเกส - บราซิล)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Română (โรมาเนีย)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Русский (รัสเซีย)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Suomi (ฟินแลนด์)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Svenska (สวีเดน)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "turkish"} ).text( 'Türkçe (ตุรกี)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Tiếng Việt (เวียดนาม)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Українська (ยูเครน)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "thai" )
		...
		Modal = ShowConfirmDialog( 'เปลี่ยนภาษา', $LanguageForm, 'เปลี่ยนภาษา' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
