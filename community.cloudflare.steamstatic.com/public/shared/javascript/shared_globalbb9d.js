...
{
	ShowConfirmDialog( 'Vous avez Steam ?', 'Il semblerait que Steam ne soit pas lancé sur votre machine. Souhaitez-vous utiliser le chat Web ?',
		'Utiliser le chat Web' , null, 'Télécharger Steam' ).done( function( choice ) {
			if ( choice == 'OK' )
				fnLaunchWebchat();
			else
	...
	.fail( function( jqxhr ) {
		...
		switch ( responseJSON.success )
		{
			case 21:
				...
				break;
			case 24:
				ShowAlertDialog( 'Erreur',
					'Votre compte ne répond pas aux exigences pour utiliser cette fonctionnalité. <a class="whiteLink" href="https://help.steampowered.com/fr/wizard/HelpWithLimitedAccount" target="_blank" rel="noreferrer">Visitez le site du Support Steam</a> pour plus d\'informations.'
				);
				break;
			case 15:
				ShowAlertDialog( 'Erreur', 'Vous n\'avez pas la permission d\'effectuer cette action.' );
				break;
			default:
				ShowAlertDialog( 'Erreur', 'Une erreur est survenue lors du traitement de votre demande :' + responseJSON.success );
				break;
		}
	} );
	return false;
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	if ( scrollTopPrevious )
	{
		...
		else
		{
			...
			{
				this.m_LoadingDialog = ShowBlockingWaitDialog( 'Veuillez patienter', 'Retour à l\'endroit précédent où vous étiez sur cette page...' );
			}
			window.scrollTo( 0, scrollTopPrevious );
		}
	}
...
function LockFamilyView( bStore )
{
	...
	ShowConfirmDialog( 'Revenir au mode famille',
		'Voulez-vous vraiment revenir au mode famille ?',
		'Revenir au mode famille'
	).done( function() {
		ShowBlockingWaitDialog( 'Revenir au mode famille' );
			...
			if ( typeof rgSuggestions[i][1] !== 'undefined' )
			{
				switch ( parseInt( rgSuggestions[i][1] ) )
				{
					case 1:
						...
						suggestionText = '[Jeu] ' + suggestionText;
						break;
					case 2:
						suggestionText = '[Application] ' + suggestionText;
						break;
					case 4:
						suggestionText = '[Outil] ' + suggestionText;
						break;
					case 8:
						resultClass += ' app_Demo';
						suggestionText = '[Démo] ' + suggestionText;
						break;
					case 16:
					...
					case 32:
						...
						break;
					case 256:
						suggestionText = '[Configuration] ' + suggestionText;
						break;
					case 2048:
						resultClass += ' app_Movie';
						suggestionText = '[Vidéo] ' + suggestionText;
						break;
					case 8192:
						resultClass += ' app_Music';
						suggestionText = '[Musique] ' + suggestionText;
						break;
					case 65536:
						resultClass += ' app_Beta';
						suggestionText = '[Bêta] ' + suggestionText;
						break;
				}
			}
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Exemple de produits', 'Veuillez patienter pendant que nous cherchons des exemples de produits...' );
	...
	$J.get( 'https://store.steampowered.com/search/results/',
		...
		{
			content.append( $J( "<div>", { class: "no_items", text: 'Aucun produit trouvé' } ) );
		}

		var dialog = ShowAlertDialog( 'Exemple de produits', content );
	} );
}
