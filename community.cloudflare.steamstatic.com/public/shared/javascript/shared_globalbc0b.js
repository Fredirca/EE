...
{
	ShowConfirmDialog( 'Tens Steam?', 'Não conseguimos encontrar o Steam aberto no teu computador. Gostarias de abrir o chat da web?',
		'Usar chat da web' , null, 'Transfere o Steam' ).done( function( choice ) {
			if ( choice == 'OK' )
				fnLaunchWebchat();
			else
		...
		switch ( responseJSON.success )
		{
			...
			case 24:
				ShowAlertDialog( 'Erro',
					'A tua conta não cumpre os requisitos para a utilização desta funcionalidade. <a class="whiteLink" href="https://help.steampowered.com/pt/wizard/HelpWithLimitedAccount" target="_blank" rel="noreferrer">Visita o Suporte Steam</a> para mais informações.'
				);
				break;
			case 15:
				ShowAlertDialog( 'Erro', 'Não tens permissão para realizar essa ação.' );
				break;
			default:
				ShowAlertDialog( 'Erro', 'Ocorreu um erro ao processar o teu pedido:' + responseJSON.success );
				break;
		}
	} );
	return false;
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	if ( scrollTopPrevious )
	{
		...
		else
		{
			...
			{
				this.m_LoadingDialog = ShowBlockingWaitDialog( 'Por favor, aguarda', 'A voltar para onde estavas nesta página...' );
			}
			window.scrollTo( 0, scrollTopPrevious );
		}
	}
...
function LockFamilyView( bStore )
{
	...
	ShowConfirmDialog( 'Voltar ao Modo Família',
		'Voltar ao Modo Família?',
		'Voltar ao Modo Família'
	).done( function() {
		ShowBlockingWaitDialog( 'Voltar ao Modo Família' );
			...
			if ( typeof rgSuggestions[i][1] !== 'undefined' )
			{
				switch ( parseInt( rgSuggestions[i][1] ) )
				{
					case 1:
						...
						break;
					case 2:
						suggestionText = '[Aplicação] ' + suggestionText;
						break;
					case 4:
					...
					case 8:
						...
						break;
					case 16:
						suggestionText = '[Conteúdo multimédia] ' + suggestionText;
						break;
					case 32:
						resultClass += ' app_DLC';
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Exemplos de produtos', 'A procurar exemplos de produtos, por favor aguarda...' );
