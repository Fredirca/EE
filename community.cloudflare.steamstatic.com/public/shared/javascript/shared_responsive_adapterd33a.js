...
function Responsive_BuildChangeLanguageOption( $MenuItem )
{
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "bulgarian"} ).text( 'Български' )
		);
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( 'Опростен китайски (简体中文)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( 'Традиционен китайски (繁體中文)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "japanese"} ).text( 'Японски (日本語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "koreana"} ).text( 'Корейски (한국어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "thai"} ).text( 'Тайландски (ไทย)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Чешки (Čeština)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Датски (Dansk)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Немски (Deutsch)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'Английски (English)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Испански — Испания (Español — España)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Испански — Латинска Америка (Español — Latinoamérica)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "greek"} ).text( 'Гръцки (Ελληνικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Френски (Français)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Италиански (Italiano)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Унгарски (Magyar)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Холандски (Nederlands)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Норвежки (Norsk)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Полски (Polski)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Португалски (Português)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Бразилски португалски (Português — Brasil)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Румънски (Română)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Руски (Русский)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Финландски (Suomi)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Шведски (Svenska)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "turkish"} ).text( 'Турски (Türkçe)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Виетнамски (Tiếng Việt)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Украински (Українська)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "bulgarian" )
		...
		Modal = ShowConfirmDialog( 'Смяна на езика', $LanguageForm, 'Смяна на езика' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
