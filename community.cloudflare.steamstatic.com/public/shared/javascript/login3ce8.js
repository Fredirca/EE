...
CLoginPromptManager.prototype.DoLogin = function()
{
	...
	$J.post( this.m_strBaseURL + 'getrsakey/', this.GetParameters( { username: username } ) )
		...
		.fail( function () {
			ShowAlertDialog( 'Ошибка', 'При подключении к серверам Steam возникла проблема. Повторите попытку позже.' );
...
CLoginPromptManager.prototype.OnRSAKeyResponse = function( results )
{
	...
	else
	{
		if ( results.message )
		{
			ShowAlertDialog( 'Ошибка', results.message );
		}
		else
		{
			ShowAlertDialog( 'Ошибка', 'При подключении к серверам Steam возникла проблема. Повторите попытку позже.' );
...
CLoginPromptManager.prototype.OnLoginResponse = function( results )
{
	...
	else
	{
		...
		{
			ShowDialog( 'Технология Intel® Identity Protection', this.m_$ModalIPT.show() ).always( $J.proxy( this.ClearLoginForm, this ) );
		}
...
CLoginPromptManager.prototype.StartTwoFactorAuthProcess = function()
{
	...
	var _this = this;
	this.m_TwoFactorModal = ShowDialog( 'Мобильная аутентификация Steam Guard', this.m_$ModalTwoFactor.show() )
