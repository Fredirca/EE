...
jQuery( function($) {
	...
	if ( $LocalMenuContent.length )
	{
		...
		GPNavUpdateActionDescriptions( $('#responsive_page_template_content'), { onOptionsActionDescription:'メニュー' } );
...
function Responsive_BuildChangeLanguageOption( $MenuItem )
{
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "japanese"} ).text( '日本語' )
		);
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( '简体中文（簡体字中国語）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( '繁體中文（繁体字中国語）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "koreana"} ).text( '한국어 (韓国語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "thai"} ).text( 'ไทย (タイ語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "bulgarian"} ).text( 'български (ブルガリア語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Čeština（チェコ語）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Dansk (デンマーク語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Deutsch (ドイツ語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'English (英語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español - España (スペイン語 - スペイン)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Español - Latinoamérica (スペイン語 - ラテンアメリカ)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "greek"} ).text( 'Ελληνικά (ギリシャ語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Français (フランス語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Italiano (イタリア語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Magyar（ハンガリー語）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Nederlands (オランダ語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Norsk (ノルウェー語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Polski (ポーランド語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Português（ポルトガル語－ポルトガル）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Português - Brasil (ポルトガル語 - ブラジル)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Română（ルーマニア語）' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Русский (ロシア語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Suomi (フィンランド語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Svenska (スウェーデン語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "turkish"} ).text( 'Türkçe (トルコ語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Tiếng Việt (ベトナム語)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Українська (ウクライナ語)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "japanese" )
		...
		Modal = ShowConfirmDialog( '言語を変更', $LanguageForm, '言語を変更' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
