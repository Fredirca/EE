...
jQuery( function($) {
	...
	if ( $LocalMenuContent.length )
	{
		...
		GPNavUpdateActionDescriptions( $('#responsive_page_template_content'), { onOptionsActionDescription:'메뉴' } );
...
function Responsive_BuildChangeLanguageOption( $MenuItem )
{
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "koreana"} ).text( '한국어' )
		);
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( '简体中文(중국어 간체)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( '繁體中文(중국어 번체)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "japanese"} ).text( '日本語(일본어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "thai"} ).text( 'ไทย(태국어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "bulgarian"} ).text( 'Български(불가리아어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Čeština(체코어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Dansk(덴마크어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Deutsch(독일어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'English(영어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español - España(스페인어 - 스페인)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Español - Latinoamérica(스페인어 - 중남미)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "greek"} ).text( 'Ελληνικά(그리스어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Français(프랑스어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Italiano(이탈리아어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Magyar(헝가리어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Nederlands(네덜란드어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Norsk(노르웨이어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Polski(폴란드어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Português(포르투갈어 - 포르투갈)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Português - Brasil(포르투갈어 - 브라질)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Română(루마니아어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Русский(러시아어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Suomi(핀란드어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Svenska(스웨덴어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "turkish"} ).text( 'Türkçe(튀르키예어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Tiếng Việt(베트남어)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Українська(우크라이나어)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "koreana" )
		...
		Modal = ShowConfirmDialog( '언어 변경', $LanguageForm, '언어 변경' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
