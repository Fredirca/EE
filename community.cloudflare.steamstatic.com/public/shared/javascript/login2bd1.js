...
CLoginPromptManager.prototype.DoLogin = function()
{
	...
	$J.post( this.m_strBaseURL + 'getrsakey/', this.GetParameters( { username: username } ) )
		...
		.fail( function () {
			ShowAlertDialog( 'Fout', 'Er is een probleem opgetreden bij het communiceren met de Steam-servers. Probeer het later opnieuw.' );
...
CLoginPromptManager.prototype.OnRSAKeyResponse = function( results )
{
	...
	else
	{
		if ( results.message )
		{
			ShowAlertDialog( 'Fout', results.message );
		}
		else
		{
			ShowAlertDialog( 'Fout', 'Er is een probleem opgetreden bij het communiceren met de Steam-servers. Probeer het later opnieuw.' );
