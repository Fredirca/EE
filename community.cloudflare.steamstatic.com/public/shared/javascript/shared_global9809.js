...
{
	ShowConfirmDialog( '安裝 Steam 了嗎？', '您的電腦似乎未執行 Steam。您想要開啟網頁聊天嗎？',
		'使用網頁瀏覽器聊天' , null, '取得 Steam' ).done( function( choice ) {
			if ( choice == 'OK' )
				fnLaunchWebchat();
			else
...
function RateAnnouncement( rateURL, gid, bVoteUp, clanID )
{
	...
	.fail( function( jqxhr ) {
		...
		switch ( responseJSON.success )
		{
			...
			case 24:
				ShowAlertDialog( '錯誤',
					'您的 Steam 帳戶並未達到使用此功能的需求。 <a class="whiteLink" href="https://help.steampowered.com/zh-tw/wizard/HelpWithLimitedAccount" target="_blank" rel="noreferrer">造訪 Steam 客服網站</a>了解更多訊息。'
				);
				break;
			case 15:
				ShowAlertDialog( '錯誤', '您並無執行該動作的權限。' );
				break;
			default:
				ShowAlertDialog( '錯誤', '處理您的請求時發生錯誤：' + responseJSON.success );
				break;
		}
	} );
	return false;
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	if ( scrollTopPrevious )
	{
		...
		else
		{
			...
			{
				this.m_LoadingDialog = ShowBlockingWaitDialog( '請稍候', '正在返回您上一次瀏覽此頁面的位置…' );
			}
			window.scrollTo( 0, scrollTopPrevious );
		}
	}
...
function LockFamilyView( bStore )
{
	...
	ShowConfirmDialog( '返回至家庭監護',
		'您確定要返回至家庭監護嗎？',
		'返回至家庭監護'
	).done( function() {
		ShowBlockingWaitDialog( '返回至家庭監護' );
			...
			if ( typeof rgSuggestions[i][1] !== 'undefined' )
			{
				switch ( parseInt( rgSuggestions[i][1] ) )
				{
					case 1:
						...
						suggestionText = '[遊戲] ' + suggestionText;
						break;
					case 2:
						suggestionText = '[應用程式] ' + suggestionText;
						break;
					case 4:
					...
					case 8:
						...
						suggestionText = '[試玩版] ' + suggestionText;
						break;
					case 16:
						suggestionText = '[媒體] ' + suggestionText;
						break;
					case 32:
						resultClass += ' app_DLC';
						suggestionText = '[可下載內容] ' + suggestionText;
						break;
					case 256:
					...
					case 2048:
						...
						suggestionText = '[影片] ' + suggestionText;
						break;
					case 8192:
						resultClass += ' app_Music';
						suggestionText = '[音樂] ' + suggestionText;
						break;
					case 65536:
						resultClass += ' app_Beta';
						suggestionText = '[測試版] ' + suggestionText;
						break;
				}
			}
...
{
	var waitDialog = ShowBlockingWaitDialog( '產品範例', '我們正在尋找產品範例，請稍候…' );
	...
	$J.get( 'https://store.steampowered.com/search/results/',
		...
		{
			content.append( $J( "<div>", { class: "no_items", text: '找不到產品' } ) );
		}

		var dialog = ShowAlertDialog( '範例產品', content );
	} );
}
