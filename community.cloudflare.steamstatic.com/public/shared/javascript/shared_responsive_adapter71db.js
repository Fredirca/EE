...
jQuery( function($) {
	...
	if ( $LocalMenuContent.length )
	{
		...
		GPNavUpdateActionDescriptions( $('#responsive_page_template_content'), { onOptionsActionDescription:'Μενού' } );
	...
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "greek"} ).text( 'Ελληνικά (Greek)' )
					...
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( '简体中文 (Απλοποιημένα κινεζικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( '繁體中文 (Παραδοσιακά κινεζικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "japanese"} ).text( '日本語 (Ιαπωνικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "koreana"} ).text( '한국어 (Κορεατικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "thai"} ).text( 'ไทย (Ταϊλανδικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "bulgarian"} ).text( 'Български (Βουλγαρικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Čeština (Τσεχικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Dansk (Δανικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Deutsch (Γερμανικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'English (Αγγλικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español – España (Ισπανικά – Ισπανία)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Español – Latinoamérica (Ισπανικά – Λατινική Αμερική)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Français (Γαλλικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Italiano (Ιταλικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Magyar (Ουγγρικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Nederlands (Ολλανδικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Norsk (Νορβηγικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Polski (Πολωνικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Português (Πορτογαλικά – Πορτογαλία)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Português – Brasil (Πορτογαλικά – Βραζιλία)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Română (Ρουμανικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Русский (Ρωσικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Suomi (Φινλανδικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Svenska (Σουηδικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "turkish"} ).text( 'Türkçe (Τουρκικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Tiếng Việt (Βιετναμικά)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Українська (Ουκρανικά)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "greek" )
		...
		Modal = ShowConfirmDialog( 'Αλλαγή γλώσσας', $LanguageForm, 'Αλλαγή γλώσσας' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
