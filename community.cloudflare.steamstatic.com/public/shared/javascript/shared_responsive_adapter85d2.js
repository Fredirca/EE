...
function Responsive_BuildChangeLanguageOption( $MenuItem )
{
	$MenuItem.click( function() {
		...
		$SelectBox.append(
			$J('<option/>', {value: "turkish"} ).text( 'Türkçe' )
		);
					$SelectBox.append(
				$J('<option/>', {value: "schinese"} ).text( '简体中文 (Basitleştirilmiş Çince)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "tchinese"} ).text( '繁體中文 (Geleneksel Çince)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "japanese"} ).text( '日本語 (Japonca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "koreana"} ).text( '한국어 (Korece)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "thai"} ).text( 'ไทย (Tayca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "bulgarian"} ).text( 'Български (Bulgarca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "czech"} ).text( 'Čeština (Çekçe)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "danish"} ).text( 'Dansk (Danca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "german"} ).text( 'Deutsch (Almanca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "english"} ).text( 'English (İngilizce)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "spanish"} ).text( 'Español - España (İspanyolca - İspanya)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "latam"} ).text( 'Español - Latinoamérica (İspanyolca - Latin Amerika)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "greek"} ).text( 'Ελληνικά (Yunanca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "french"} ).text( 'Français (Fransızca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "italian"} ).text( 'Italiano (İtalyanca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "hungarian"} ).text( 'Magyar (Macarca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "dutch"} ).text( 'Nederlands (Hollandaca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "norwegian"} ).text( 'Norsk (Norveççe)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "polish"} ).text( 'Polski (Lehçe)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "portuguese"} ).text( 'Português (Portekizce - Portekiz)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "brazilian"} ).text( 'Português - Brasil (Portekizce - Brezilya)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "romanian"} ).text( 'Română (Rumence)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "russian"} ).text( 'Русский (Rusça)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "finnish"} ).text( 'Suomi (Fince)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "swedish"} ).text( 'Svenska (İsveççe)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "vietnamese"} ).text( 'Tiếng Việt (Vietnamca)' )
			);
					$SelectBox.append(
				$J('<option/>', {value: "ukrainian"} ).text( 'Українська (Ukraynaca)' )
			);
		
		var Modal = null;

		var fnChangeLanguage = function() {
			if ( $SelectBox.val() != "turkish" )
		...
		Modal = ShowConfirmDialog( 'Dili değiştir', $LanguageForm, 'Dili değiştir' ).done( fnChangeLanguage );
		$SelectBox.focus();
	});
}
